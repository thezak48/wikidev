...
Ja, Radarr stöder att lagra alla filmfiler i en enda mapp. För att göra detta, gå till inställningarna för kvalitetsprofilen och välj "Avancerade inställningar". I avsnittet "Filhantering" kan du ange en anpassad sökväg för att lagra alla filer i en enda mapp. Du kan använda variabler som `{Movie Title}` och `{Quality Full}` i sökvägen för att organisera filerna på ett strukturerat sätt. När du har konfigurerat sökvägen och sparat ändringarna kommer Radarr att flytta befintliga filer till den nya mappen och alla nya nedladdade filer kommer också att sparas där.

- Nej, och anledningen är att Radarr är en avgrening av [Sonarr](/sonarr), där varje show har en mapp. Denna begränsning är en känd smärtpunkt för många användare och kanske kommer att åtgärdas i en framtida version. Observera att det inte är en enkel ändring och kräver i praktiken en komplett omskrivning av backenden.
- [GitHub-problemet för anpassade mappar](https://github.com/Radarr/Radarr/issues/153) täcker tekniskt sett denna begäran, men det finns ingen garanti för att alla filer för en film i en mapp kommer att implementeras vid den tiden.
- En liten hackig lösning beskrivs nedan. Observera att du inte får trigga en omgenomsökning i Radarr, annars kommer den att visas som saknad och oavsett kommer filmen aldrig att uppgraderas.
...
- ![Nuvarande Develop/Beta](https://img.shields.io/badge/dynamic/json?color=f5f5f5&style=flat-square&label=Develop&query=%24%5B0%5D.version&url=https://radarr.servarr.com/v1/update/develop/changes) - (Beta): Detta är kanten för testning. Släppt efter att ha testats i nattliga för att säkerställa att det inte finns några omedelbara problem. Nya funktioner och buggfixar släpps här först efter nattliga. Det kan betraktas som halvstabilt, men är fortfarande `beta`. Den här versionen kommer att få uppdateringar antingen veckovis eller varannan vecka beroende på utveckling.
...
- ![Nuvarande Nightly/Ostabilt](https://img.shields.io/badge/dynamic/json?color=f5f5f5&style=flat-square&label=Nightly&query=%24%5B0%5D.version&url=https://radarr.servarr.com/v1/update/nightly/changes) - (Alfa/Ostabilt): Detta är den senaste versionen. Den släpps så snart koden har commitats och passerar alla automatiserade tester. Den här versionen kan ännu inte ha använts av oss eller andra användare. Det finns ingen garanti för att den ens kommer att fungera i vissa fall. Den här grenen rekommenderas endast för avancerade användare. Problem och egen undersökning förväntas i denna gren. ***Använd den här grenen endast om du vet vad du gör och är villig att få dina händer smutsiga för att återställa en misslyckad uppdatering.*** Den här versionen uppdateras omedelbart.
...
### Kan jag uppdatera Radarr inuti min Docker-behållare?

- *Tekniskt sett, ja.* **Men du bör absolut inte göra det.** Det är en grundläggande princip för Docker. Databasproblem kan uppstå om du uppgraderar din installation till den senaste `nightly`, men sedan uppdaterar Docker-behållaren själv (eventuellt nedgraderar till en äldre version).
...
- Om du inte följer dessa instruktioner kan din Radarr bli oanvändbar eller kasta fel. Du har blivit varnad. Om följande fel uppstår använder du en nyare databas med en äldre \*Arr-version som inte stöds. Uppgradera \*Arr till den version du tidigare använde eller en nyare.
...
- Hitta och rätta till den film som redan är tilldelad den sökvägen.
  ...
  - Sortera och hitta filmen vid den noterade problematiska sökvägen.
...
> Samma process gäller också för att flytta/ändra sökvägar för filmer. Om du bara uppdaterar sökvägar i Radarr och inte behöver flytta filerna, välj inte "Ja, flytta filer" i steg 5.
{.is-info}
...
- För närvarande kräver Radarr att varje film finns i en mapp med formatet `Filmens titel (År)/`, valfritt är `_` eller `.` giltiga separatorer. För att underlätta korrekt identifiering av kvalitet och upplösning vid import är det bäst med ett filnamn som `Filmens titel (År) [Kvalitet-Upplösning].ext`, återigen är `_` eller `.` giltiga separatorer.

  - Ett användbart verktyg för att göra dessa ändringar i din samling är [filebot](http://www.filebot.net/#download), som har en betalversion i både Apple och Windows-butikerna, men kan hittas gratis på deras äldre [SourceForge](https://sourceforge.net/projects/filebot/files/latest/download) webbplats. Det har både ett GUI och CLI, så du kan använda det du är bekväm med. För det ovanstående exemplet expanderar `{ny}` till `Namn (År)` och `{vf}` ger upplösningen som `1080p`. Det finns inget att härleda kvaliteten, så du kan fejka det med hjälp av `{ny}/{ny} [{dim[0] >= 1280 ? 'Bluray' : 'DVD'}-{vf}]`, vilket kommer att namnge allt lägre än 720p till `[DVD-572p]` och större eller lika med 720p som `[Bluray-1080p]`.
...
## Filmmappar med felaktigt namn

- Namngivningen av filmmappar baseras på metadata (namn/år) vid tidpunkten då filmen lades till. Om du lade till en film innan den släpptes kan du behöva använda ovanstående trick för att uppdatera namnet på filmmappen så att det stämmer överens med de nya aktuella uppgifterna.
- Även om dina filmer redan finns i mappar kanske inte mapparna är namngivna korrekt. Mappnamnet bör vara `Filmens titel (År)`, att ha titeln och året i mappens namn är kritiskt just nu.
  ...
  - Exempel som fungerar, men kräver manuell hantering:
    ...
    - Efter genre: `/mnt/Filmer/Crime, Drama, Sci-Fi/A Clockwork Orange (1971)/A Clockwork Orange (1971) [Bluray-1080p].mkv`
  ...
  - Exempel som inte fungerar:
    - Enkel mapp: `/mnt/Barnfilmer/Frozen (2013) [Bluray-1080p].mkv`
      - För närvarande måste filmer helt enkelt finnas i en mapp med samma namn som filmen. Det finns inget sätt att komma runt detta förrän utvecklingsarbete är klart för att lägga till den här funktionen.
    - **Filmmappsnamn i formatet** från v0.2 som inkluderar **Filme**genskaper i **filmmappens** namn som ``{Movie.Title}.{Release Year}.{Quality.Full}-{MediaInfo.Simple}{`Release.Group}`` kommer inte att fungera i v3.
...
- Den främsta inställningen är "Sök igenom filmmapp efter uppdatering". Om din disk I/O-användning ökar under en uppdatering kan du ändra inställningen för att söka igenom manuellt.
  ...
  - Om du tar bort filmfiler manuellt eller via Plex eller ett annat tredjepartsprogram, ställ inte in detta på "Aldrig".
- Den andra inställningen som kan ändras är "Analysera videofiler", vilket rekommenderas om du använder tdarr eller på annat sätt ändrar dina filer externt. Om du inte gör det kan du tryggt inaktivera "Analysera videofiler" för att minska lite I/O.
...
- Detta är enkelt, se denna länk för mer information [här](https://support.apple.com/sv-se/guide/mac-help/open-a-mac-app-from-an-unidentified-developer-mh40616/mac) ![Utvecklaren kan inte verifieras](developer-cannot-be-verified.png "Utvecklaren kan inte verifieras")
...
> \* För Radarr-användare som upplever detta efter uppgradering till v4.X-versioner. v4-versioner gör flera långtgående migreringar på grund av att om din databas tidigare hade korruption på någon plats (som kanske inte var upptäckbar tidigare när Radarr kördes) kommer migreringen att misslyckas. Detta kommer att orsaka att Radarr inte kan starta. Det är troligt att alla dina säkerhetskopior också är korrupta, så att bara återställa dem kommer sannolikt inte att lösa problemet.
> \* Om den migrerade databasen inte kan öppnas eller inte kan återställas, gör en kopia av databasen från en nyligen gjord säkerhetskopia och tillämpa databasåterställningsprocessen på den filen och försök sedan starta Radarr med den återställda säkerhetskopian. Den bör då migrera utan problem.
{.is-warning}

- **Felmeddelanden om `Fel vid skapande av logg-databas` indikerar problem med logs.db**
  - Detta kan snabbt åtgärdas genom att byta namn på eller ta bort databasen. Loggdatabasen innehåller oviktig information om kommandohistorik och installationshistorik för uppdateringar, samt Info-, Warn- och Error-poster.
...
- Detta innebär att din SQLite-databas som lagrar det mesta av informationen för Radarr är korrupt. Dina alternativ är att försöka (ett) säkerhetskopiering(ar), försöka återställa den befintliga databasen, försöka återställa säkerhetskopiorna eller om allt annat misslyckas börja om med en helt ny databas.
- Detta felmeddelande kan visas om databasfilen inte kan skrivas av användaren/gruppen som \*Arr körs som. Behörigheter kan vara orsaken till problemet endast för nya installationer, migrerade installationer till en ny server, om du nyligen har ändrat behörigheterna för din appdata-katalog eller om du har ändrat användaren och gruppen som \*Arr körs som.
- Ditt bästa och första alternativ är att [försöka återställa från en säkerhetskopia](#hur-säkerhetskopierar-och-återställer-jag-min-radarr). Men för användare som får detta efter uppgradering till v4 är det mycket osannolikt att själva säkerhetskopieringen kommer att fungera och du måste prova de andra återställningsmetoderna som nämns.
- Du kan också försöka återställa din databas. Detta är vanligtvis det enda alternativet när detta problem uppstår efter en uppdatering. Prova [sqlite3-kommandot `.recover`](/useful-tools#recovering-a-corrupt-db)
  - Om ditt sqlite inte har `.recover` eller om du vill ha ett mer GUI-vänligt sätt (t.ex. Windows) kan du följa [våra instruktioner på denna wiki](/useful-tools#recovering-a-corrupt-db-ui)
...
## Jag använder Radarr på en Mac och det slutade plötsligt fungera. Vad har hänt?
...
- Försök sedan starta och se om det fungerar. Om det inte fungerar behöver du ytterligare support. Skriv ett inlägg i vår [subreddit /r/radarr](http://reddit.com/r/radarr) eller kom in på [vår discord](https://radarr.video/discord) för hjälp.
...
- För Windows: Kort sagt, användaren som \*Arr körs som (om det är en tjänst) eller under (om det är en brickapplikation) kan inte komma åt filvägen på fjärrservern. Det kan bero på olika orsaker, men den vanligaste är att \*Arr körs som en tjänst, vilket orsakar de problem som beskrivs nedan.
...
## Hur ändrar jag från Windows-tjänsten till en brickapplikation?

1. Stäng av Radarr
1. Kör serviceuninstall.exe som finns i Radarr-mappen
1. Kör `Radarr.exe` som administratör en gång för att ge det rätt behörigheter och öppna brandväggen. När det är klart kan du stänga det och köra det normalt.
...
- Radarr kommer att behöva flytta filer från där nedladdaren placerar dem till den slutliga platsen, så detta innebär att den måste ha läs- och skrivåtkomst till både käll- och destinationsmappen och filerna.
...
- Det är EasyPrivacy-blocklistan. De blockerar i princip alla URL:er med /api/log? i dem. Titta igenom listan och avgör om du tycker att det är en rimlig idé att blockera alla URL:er i den, det finns dussintals URL:er där som potentiellt bryter webbplatser. Du valde det i din annonsblockerare. Det enkla sättet är att lägga till domänen som Radarr körs på i whitelistan. Men jag rekommenderar ändå att du kontrollerar listan.
...
- Se till att alternativt lyssningsport (Avancerat => Webbgränssnitt) inte är samma som lyssningsporten (Anslutningar)
- Vi rekommenderar att du ändrar alternativet för alternativt lyssningsport för webbgränssnittet för att inte störa portvidarebefordran för anslutningar.

## Jag fick ett popup-fönster som sa att config.xml var korrupt, vad gör jag nu?

- Radarr kunde inte läsa din konfigurationsfil vid start eftersom den på något sätt blev korrupt. För att komma online igen måste du ta bort `.xml` i din [appdata-katalog](/radarr/appdata-directory), när den är borttagen startar den på standardporten (7878), du bör nu konfigurera om eventuella inställningar du konfigurerade på sidan för allmänna inställningar.
...
- Din nedladdningsklient har slutat fungera och du får ett fel som "Localhost är ett ogiltigt certifikat"?
  ...
  - Om din nedladdningsklient och Radarr körs på samma maskin finns det ingen anledning att använda HTTPS, så det enklaste sättet är att inaktivera SSL för anslutningen. De flesta skulle hålla med om att det inte är nödvändigt i ett lokalt nätverk heller. Det är möjligt att inaktivera certifikatvalidering i avancerade inställningar om du vill behålla en osäker SSL-konfiguration.
...
- Med andra ord kan det göra att \*ARR-programmen (Lidarr, Prowlarr, Radarr, Readarr och Lidarr) blir oanvändbara i vissa fall på grund av att tjänsterna inte är tillgängliga om du sätter dem bakom en VPN.

> **För att vara tydlig handlar det inte om ifall VPN kommer att orsaka problem med \*ARR-programmen, utan när: bildleverantörer kommer att blockera dig och cloudflare finns framför de flesta \*ARR-servrar (uppdateringar, metadata osv.) och kan också blockera dig**
{.is-warning}
...
- I vissa fall (t.ex. brittiska internetleverantörer) kan du behöva sätta din torrentnedladdningsklient bakom en VPN och Jackett/Prowlarr enligt följande:
  - sätt Jackett bakom VPN:en och se till att split tunneling tillåter lokal åtkomst
  - för Prowlarr konfigurerar du din VPN-klient för att tillhandahålla en proxy och lägger till proxyn i Inställningar => Indexers. Ge proxyn en tagg och ge samma tagg till eventuella indexers som behöver använda den.
    - Om det absolut krävs och om din VPN inte har något sätt att skapa en proxy kan du sätta Prowlarr bakom VPN:en och se till att split tunneling tillåter lokal åtkomst.
...
- Radarr använder det äldsta **biografiska premiärdatumet** och det äldsta **premiärdatumet** för att matcha.
  - Observera att om det inte finns något biografiskt premiärdatum kommer logiken att använda det äldsta fysiska eller digitala premiärdatumet.
- Om en film saknar ett digitalt, fysiskt, biografiskt eller premiärdatum bör TMDb uppdateras.
- [TMDb's Contribution Bible](https://www.themoviedb.org/bible/movie/59f3b16d9251414f20000009#59f73d3c9251416e71000013) nämner följande om deras releastyper.
  - **Premiär** - En premiärvisning kan vara en festivalvisning (t.ex. TIFF) eller ett premiärevenemang med skådespelare och filmteam i en storstad (t.ex. LA, London, Toronto).
  - **Biografisk** - Kan användas för den ursprungliga premiären och alla efterföljande officiella premiärer. Används för breda eller mättade premiärer. I USA anses 600-1 999 biografer vara en bred premiär och 2 000+ biografer anses vara en mättad premiär.
  - **Biografisk (begränsad)** - Begränsad biografisk premiär är en distributionsstrategi för filmer där en ny film släpps på ett fåtal biografer i ett land, vanligtvis i större storstadsområden. I USA är antalet biografer färre än 600.
  - **Fysisk** - Inkluderar alla VHS-, DVD- och Blu-ray-utgåvor.
  - **Digital** - Alla relevanta utgåvor kan läggas till, inklusive strömningstjänster, VOD-uthyrning eller -köp. Digitala visningar, inklusive online-filmfestivaler och virtuella biografvisningar, räknas också som digitala utgåvor.

## Hur hanterar Radarr utländska filmer eller utländska titlar?

> [TRaSH's Custom Format Language Guide](https://trash-guides.info/Radarr/Tips/How-to-setup-language-custom-formats/#how-to-setup-language-custom-formats) kan vara användbart för att hjälpa till att få filmer på det/den språk du vill ha.{.is-info}

> Från och med 2023-02-12 kommer Radarrs metadatacache att börja betrakta en films ursprungsspråk som TMDbs talade språk om och endast om det endast finns ett talat språk för filmen på TMDb; annars kommer filmens ursprungliga TMDb-språk att användas.
{.is-warning}

## Sökningar med ID

- **Indexer som stöder sökningar baserade på ID** - Sökningar på indexers och spårare som stöder ID-baserade sökningar (TMDbId, IMDbId osv.) - som många Usenet-indexers och många privata Torrent-spårare - textfrågor används inte om resultat returneras för en ID-baserad sökning. Om resultat returneras kommer det inte att falla tillbaka till en namn/textsökning. Om du saknar resultat från din indexer beror det på att de har släppen associerade med fel film-ID.
  - Språket för släppet kan också härledas från indexerarens eller spårarens släpps språk i resultatet om det tillhandahålls, istället för att tolkas från namnet

## Textsökningar

- **Indexer som inte stöder ID-baserade sökningar eller ID-baserade sökningar utan resultat** - Sökningar kommer att använda filmens ursprungliga titel, engelska titel och översatt titel från [vilka språk du har valt i filmens kvalitetsprofil och eventuella anpassade format med poäng i kvalitetsprofilen större än noll](https://trash-guides.info/Radarr/Tips/How-to-setup-language-custom-formats/).
- Parsing (dvs. importering) letar efter en matchning i alla översättningar och alternativa titlar.
  - Språket för släppet kan också härledas från indexerarens eller spårarens släpps språk i resultatet om det tillhandahålls, istället för att tolkas från namnet

## Att få utländska filmer

- För att få en film på ett främmande språk kan du ställa in filmens kvalitetsprofil på Original (Filmens ursprungsspråk\*), ett specifikt språk för den profilen eller `Any` och skapa och poängsätta [anpassade format med språkvillkor för att bestämma vilket språk som ska hämtas - se TRaSH's Guide för detaljer](https://trash-guides.info/Radarr/Tips/How-to-setup-language-custom-formats/).
- Observera att detta inte inkluderar några indexerade språk som är konfigurerade som `multi` i indexerarens inställningar.
...
`multi` endast är filmens språk och **INTE** engelska som i tidigare versioner.
  - Användare kan justera sina inställningar per indexerare för att ange vilket/vilka språk `multi` indikerar
- Observera att `multi`-definitioner bara hjälper till för att tolka släpp och inte för utländska titlar eller filmsökningar.
...
- Jacketts `/all`-endpunkt är bekväm, men det är dess enda fördel. Allt annat kan orsaka problem, så det krävs att varje spårare läggs till individuellt. Alternativt kan du överväga det Jackett- och NZBHydra2-alternativet [Prowlarr](/prowlarr)
...
- Jacketts /all-endpunkt är bekväm, men det är dess enda fördel. Allt annat kan orsaka problem, så nu krävs det att varje spårare läggs till individuellt.
...
- Att använda /all-endpunkten har inga fördelar, bara nackdelar:
  ...
  - indexerarens specifika kategorier (\>= 100000) kan inte användas.
...
- Kolla in [Prowlarr](/prowlarr) som kan synkronisera indexerare till \*Arr och kommer från Lidarr/Radarr/Readarr-utvecklingsteamet.
- Kolla in [NZBHydra2](https://github.com/theotherp/nzbhydra2) som kan synkronisera indexerare till \*Arr. Men använd inte deras enskilda samlingsendpunkt och använd `multi` om synkronisering kommer att användas.
...
> Hårdlänkar är aktiverade som standard. [En hårdlänk kommer inte att använda något extra diskutrymme.](https://trash-guides.info/Hardlinks/Hardlinks-and-Instant-Moves/) Filsystemet och monteringspunkterna måste vara desamma för din slutförda nedladdningsmapp och din mediebiblioteksmapp. Om skapandet av hårdlänken misslyckas eller om din konfiguration inte stöder hårdlänkar kommer kopieras filen istället.
{.is-info}
...
- Se till att du **inte** inkluderar `proxy_set_header Connection "Upgrade";` som föreslagits i nginx-dokumentationen. **DET HÄR KOMMER INTE ATT FUNGERA**
- [Se detta ASP.NET Core-problem](https://github.com/aspnet/AspNetCore/issues/17081)
- Om du använder en CDN som Cloudflare måste websockets vara aktiverade för att tillåta websocket-anslutningar.
