...
1. 创建完 Jail 后，它将自动启动。还需要设置一个属性，以便 Sonarr 可以看到挂载的媒体位置的存储空间。在服务器上打开 root shell 并输入以下命令：
...
假设已配置了 iocage 网络桥接（vnet）（<https://iocage.readthedocs.io/en/latest/networking.html>）
...
返回到 jails 列表，找到刚创建的 `sonarr` jail，然后点击 "Shell"
...
创建 Sonarr 用户和组（如果不想使用用户/组 'sonarr'，可以根据首选项进行更改）
...
现在我们已经安装了一些软件，还需要进行一些步骤。

### 服务设置

是时候启用服务了，但在此之前，请注意：

更新程序默认处于禁用状态。`pkg-message` 提供了启用更新程序的说明，但请记住：当内置更新程序替换文件时，这可能会破坏 Sonarr 的 `pkg check -s` 和 `pkg remove` 等功能。
...
`sonarr` 默认将其数据、配置、日志和 PID 文件存储在 `/usr/local/sonarr` 中。服务文件将在该位置创建它并拥有它，**仅当该位置不存在时**。如果您想将这些文件存储在其他位置（例如，挂载到 jail 中以便更轻松地进行快照），则需要使用 `sysrc` 进行更改

```shell
sysrc sonarr_data_dir="DIR_YOU_WANT"
```

提醒：如果您使用的是现有位置，则需要手动将所有权更改为 UID/GID `sonarr` 使用的值，并/或将 `sonarr` 添加到具有写入访问权限的 GID。
...
如果一切按计划进行，Sonarr 应该在 jail 的 IP 上（端口 8989）运行！
